...
test_that("new_workflowstep sets name and label correctly", {
  param1 <- new_operationparam(4, 1, "x", "foo")
  param2 <- new_operationparam(4, 2, "split", ", ")
  step <- new_workflowstep(id = 4, operation = "strsplit", params = list(param1, param2))
  expect_equal(step$name, "Step 4")
  expect_equal(step$label, "Step 4")
  step2 <- new_workflowstep(
    id = 5,
...
# Test run_step.workflowstep with valid operationparam
test_that("run_step.workflowstep executes operation and returns correct output", {
  ...
  state <- new_workflowstate()
  steprun <- run_step.workflowstep(step, state, path_to_folder = tempdir())
...
test_that("run_step.workflowstep errors if state is not workflowstate", {
  ...
  not_state <- list()
  expect_error(run_step.workflowstep(step, not_state, path_to_folder = tempdir()), "must be a 'workflowstate' object")
})
